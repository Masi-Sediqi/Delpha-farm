...
export default function DeviceTransferManagement() {
  ...
  const [depositPaidReasons, setDepositPaidReasons] = useJsonCollection("depositPaidReasons");
...
const [isDeletingTransfer, setIsDeletingTransfer] =
  ...
  const addRefundReason = async () => {
    const title = newRefundReason.trim();
    if (!title) return;

    const exists = depositPaidReasons.some(
      (item) => String(item.title || item.reason || "").toLowerCase() === title.toLowerCase()
    );
    const nextReasons = exists
      ...
      : [
          ...depositPaidReasons,
          {
            id: `deposit-paid-reason-${Date.now()}`,
            title,
            createdAt: new Date().toISOString(),
          },
        ];
    const saved = exists || (await setDepositPaidReasons(nextReasons));
    if (!saved) return;

    setForm((previous) => ({ ...previous, refundReason: title }));
    setNewRefundReason("");
  };
  ...
  const buildReversedTransferAssets = (transfer, baseAssets = assets) => {
  ...
  const reverseTransferEffects = async (transfer) => {
    return setAssets(buildReversedTransferAssets(transfer));
  };
...
const deleteTransferRecord = async () => {
  ...
  const editTransferRecord = (transfer) => {
    setForm({
      ...emptyForm,
    ...
    notify("Transfer loaded into the form. Save changes only if you want to update it.");
  };
  ...
  const saveTransfer = async (event) => {
    ...
    const assetBaseForSave = editTransfer
      ? buildReversedTransferAssets(editTransfer, assets)
      : assets;
    const nextAssets = assetBaseForSave.map((asset) => {
    ...
    const deviceTransfersBase = editTransfer
      ? deviceTransfers.filter((item) => keyOf(item.id) !== keyOf(editTransfer.id))
      : deviceTransfers;
    const deviceHistoryBase = editTransfer
      ? deviceHistory.filter(
          (item) =>
            keyOf(item.transferId) !== keyOf(editTransfer.transferId) &&
            keyOf(item.referenceNumber) !== keyOf(editTransfer.referenceNumber)
        )
      : deviceHistory;
    const securityDepositsBase = editTransfer
      ? securityDeposits.filter(
          (item) => keyOf(item.transferId) !== keyOf(editTransfer.transferId)
        )
      : securityDeposits;
    ...
    const transfersSaved = await setDeviceTransfers([...deviceTransfersBase, ...allTransferRecords]);
    ...
    if (editTransfer) {
      await setSecurityDeposits(securityDepositsBase);
      await removeTransferIncome(editTransfer);
      await removeTransferWasteExpense(editTransfer);
      await removeTransferCustomerDamageIncome(editTransfer);
    }
          ...
          {includesCustomerRefund && (
            <div className="device-transfer-deposit-panel device-transfer-full">
              ...
              <label className="device-transfer-full">
                Reason
                <div className="device-transfer-reason-builder">
                  <select name="refundReason" value={form.refundReason} onChange={handleChange}>
                    <option value="">Select reason</option>
                    {depositPaidReasons.map((item) => {
                      const reason = item.title || item.reason || "";
                      return reason ? (
                        <option key={item.id || reason} value={reason}>
                          {reason}
                        </option>
                      ) : null;
                    })}
                  ...
                  <input
                    ...
                    onChange={(event) => setNewRefundReason(event.target.value)}
                    placeholder="Add new reason..."
                  />
                  <button type="button" onClick={addRefundReason}>
                    Add
                  </button>
                </div>
              </label>
          ...
          <strong>
            {[
              Number(deleteTransferTarget.depositAmount || 0) > 0
                ? `Deposit Recived: ${money(deleteTransferTarget.depositAmount)} ${deleteTransferTarget.depositCurrency || "AFN"}`
                : "",
              Number(deleteTransferTarget.refundAmount || 0) > 0
                ? `Deposit Paid: ${money(deleteTransferTarget.refundAmount)} ${deleteTransferTarget.refundCurrency || deleteTransferTarget.depositCurrency || "AFN"}`
                : "",
