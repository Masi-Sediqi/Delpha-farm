...
const translations = {
  en: {
    ...
    companyRequired: "Please enter a name for the new company.",
  ...
  fa: {
    ...
    companyRequired: "لطفاً نام کمپنی جدید را وارد کنید.",
  ...
  ps: {
    ...
    companyRequired: "مهرباني وکړئ د نوي شرکت نوم ولیکئ.",
...
const defaultCompanies = [
  "ALCO", "GL", "Head", "McOLSON", "Quick Action", "Real", "Silver Turk",
  "SPM Medical", "VMK", "-VMK-", "اتومکس", "افق", "المپریک", "برستون",
  "پرنس کی مر", "ترکی", "چینایی", "دوچی اف", "راولی", "زمین فارما",
  "سوسن-G", "سیتوفا", "لندن ساینتیفیک", "مادین", "مایر", "ماکرو",
  "محصول", "مدیرج", "نجیب شفق", "نول", "تکمیل", "هونونگ فارما",
  "بست هلت", "غدیر", "کویر فارما"
];
...
function Products() {
  ...
  const [savedCompanies, setSavedCompanies] = useJsonCollection("productCompanies");
  ...
  const companies = useMemo(() => {
    const custom = savedCompanies
      .map((item) => String(typeof item === "string" ? item : item?.name || "").trim())
      .filter(Boolean);
    return [...new Set([...defaultCompanies, ...custom])];
  }, [savedCompanies]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) =>
      [product.productName, product.group, product.company, product.madeIn]
  ...
  const handleSubmit = async (event) => {
    ...
    let selectedCompany = formData.company;
    if (addingCompany) {
      selectedCompany = newCompany.trim();
      if (!selectedCompany) {
    ...
    if (addingCompany && !companies.some((company) => company.toLowerCase() === selectedCompany.toLowerCase())) {
      const companySaved = await setSavedCompanies([...savedCompanies, { id: `company-${Date.now()}`, name: selectedCompany }]);
              ...
              <div className="sales-product-form-grid">
                ...
                <div className="sales-product-field">
                  ...
                  <div className="sales-product-select-with-add">
                    ...
                    <button type="button" className="sales-product-inline-add" onClick={() => setAddingCompany((v) => !v)} title={t.addNewCompany} aria-label={t.addNewCompany}><Plus size={18} /></button>
                  </div>
                  {addingCompany && <input value={newCompany} onChange={(e) => setNewCompany(e.target.value)} placeholder={t.newCompanyPlaceholder} />}
                </div>
