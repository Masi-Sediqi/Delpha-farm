...
const handleEditPurchaseBillImageChange = (event) => {
  ...
  if (!transfersSaved) return;
    ...
    if (isIndividualAsset) {
        const otherSerials = new Set(
            (asset.identityRecords || [])
                .filter(
                    (record) =>
                        !originalRecordIds.has(String(record.id || ""))
                )
                .map((record) =>
        ...
        const otherMacAddresses = new Set(
            (asset.identityRecords || [])
                ...
                .map((record) =>
                    String(record.macAddress || "").trim().toLowerCase()
                )
                .filter(Boolean)
        );
        ...
        .map((record) => {
            ...
            return editedRecord
                ? {
                      ...
                      category: record.category || asset.category || "",
                      unitPrice,
                      purchaseCode: editMovementForm.purchaseCode || "",
                      updatedAt: new Date().toISOString(),
                  }
                : record;
        });
    ...
    if (currentQuantity + quantityDifference < 0) {
                ...
                >
                    ...
                    >
                        ...
                        <div className="asset-movement-detail-grid">
                            ...
                            <div><span>Issued</span><strong>{viewMovement.sourceName || "-"}</strong></div>
                            <div><span>Recived</span><strong>{viewMovement.destinationName || "-"}</strong></div>
...
>
                        <div className="asset-detail-modal-header">
                            <div>
                                ...
                                <p>
                                    {editMovement?.movementType === "Purchase"
                                        ? "Update purchase quantity, price, supplier, individual devices, and notes."
                                        : "Update movement date, status, person, and notes."}
                                </p>
                            </div>
                    ...
                    <input
                        ...
                        onChange={(event) => {
                            ...
                            setEditMovementForm((previous) => {
                                ...
                                const nextRecords = isIndividualAsset
                                    ? syncIdentityRecords(
                                          Number(value || 0),
                                          previous.identityRecords || []
                                      )
