...
function CustomerDetails() {
  ...
  const [disconnectActionType, setDisconnectActionType] = useState("Suspend");
  ...
  const openDisconnectModal = (actionType = "Suspend") => {
    setDisconnectActionType(actionType);
    resetDisconnectForm();
  ...
  const saveDisconnect = async (event) => {
    ...
    const isTerminateAction = disconnectActionType === "Terminate";
    const actionDate = disconnectForm.disconnectionDate;
    const now = new Date().toISOString();

    const record = {
      id: `${isTerminateAction ? "terminate" : "disconnect"}-${Date.now()}`,
      recordType: isTerminateAction ? "Terminate" : "Suspend",
      customerRecordId: customer.id || "",
      customerId: customer.customerId || "",
      customerName: customer.customerName || "",
      disconnectionDate: actionDate,
      terminationDate: isTerminateAction ? actionDate : "",
    ...
    const customersSaved = await setCustomers(
      customers.map((item) =>
        String(item.id || item.customerId) === String(customer.id || customer.customerId)
          ? {
              ...item,
              status: isTerminateAction ? "Inactive" : "Suspend",
              disconnectionStatus: recoveryGroup,
              disconnectionDate: actionDate,
              terminationDate: isTerminateAction ? actionDate : item.terminationDate,
              updatedAt: now,
            }
          : item
      )
    ...
    notify(isTerminateAction ? "Customer terminated successfully." : "Customer suspended successfully.");
  ...
  const ledgerRows = [
    ...
    ...customerDisconnections
      ...
      .map((item) => ({
        id: `${item.recordType === "Reconnect" ? "reconnect" : item.recordType === "Terminate" ? "terminate" : "disconnect"}-ledger-${item.id}`,
        type: item.recordType === "Reconnect" ? "Reconnect" : item.recordType === "Terminate" ? "Terminate" : "Suspend",
        date:
          item.recordType === "Reconnect"
            ? item.reconnectDate || item.createdAt?.slice(0, 10) || "-"
            : item.terminationDate || item.disconnectionDate || item.createdAt?.slice(0, 10) || "-",
        timeSource: item.createdAt || item.updatedAt || "",
        description:
          item.recordType === "Reconnect"
            ? `Customer reconnected. Notes: ${item.notes || "-"}`
            : item.recordType === "Terminate"
              ? `Customer terminated and devices collected. Reason: ${item.disconnectionReason || "-"}`
              : `Customer package suspended. Reason: ${item.disconnectionReason || "-"}`,
        debit: 0,
        credit: 0,
        status: item.recordType === "Reconnect" ? "Active" : item.recordType === "Terminate" ? "Inactive" : "Suspend",
        record: item,
      })),
...
const saveCustomerPayment = async (event) => {
        ...
        <div className="customer-details-header-actions">
          {String(customer.status || "").toLowerCase() === "suspend" ? (
            ...
            <>
              <button
                type="button"
                className="customer-details-terminate-btn"
                onClick={() => openDisconnectModal("Terminate")}
              >
                Terminate
              </button>
              <button
                ...
                onClick={() => openDisconnectModal("Suspend")}
              >
                Suspend
              </button>
            </>
  ...
  >
    ...
    >
      <div className="customer-details-modal-header">
        <div>
          <h3>{disconnectActionType} Customer</h3>
          <p>
            {disconnectActionType === "Terminate"
              ? "Record the terminate details and device collection status."
              : "Record the suspend details and manage the recovery status of customer devices."}
          </p>
        </div>
      ...
      <form onSubmit={saveDisconnect}>
        <div className="customer-details-form-grid customer-disconnect-form-grid">
          ...
          <div>
            ...
            <ShamsiDateInput
              ...
              placeholder={`Enter the reason for ${disconnectActionType.toLowerCase()}`}
            />
          </div>
        </div>
        ...
        <div className="customer-disconnect-notes-field">
          ...
          <textarea
            ...
            placeholder={`Enter any additional notes about this ${disconnectActionType.toLowerCase()}...`}
          />
        </div>
