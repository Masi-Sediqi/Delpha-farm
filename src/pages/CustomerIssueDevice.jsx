          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Ownership</th>
                <th>Quantity</th>
                <th>Deposit Recived / Deposit Paid</th>
                <th>Status</th>
                <th>Actions</th>
              </tr>
            </thead>
                ...
                (item) => (
                  <tr key={item.id} className={getTransferRowClass(item)}>
                    ...
                    <td>
                      {Number(item.depositAmount || 0) ||
                      Number(item.depositRefundAmount || item.refundAmount || 0) ? (
                        <div className="customer-issue-money-list">
                          {Number(item.depositAmount || 0) ? (
                            <span className="customer-issue-money-badge deposit">
                              Deposit Recived: {money(item.depositAmount)} {item.depositCurrency || "AFN"}
                            </span>
                          ) : null}
                          {Number(item.depositRefundAmount || item.refundAmount || 0) ? (
                            <span className="customer-issue-money-badge return">
                              Deposit Paid: {money(item.depositRefundAmount || item.refundAmount)}{" "}
                              {item.refundCurrency || item.depositCurrency || "AFN"}
                            </span>
                          ) : null}
                        </div>
                      ) : (
